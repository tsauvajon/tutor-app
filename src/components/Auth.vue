<template>
<div><v-card style="margin-top: 30px; padding-top: 20px; padding-bottom:40px;" hover class="ta-small-container animated fadeInUp xfast">
  <v-card-title>
    Connexion
  </v-card-title>
  <v-card-text>
...
export default {
  ...
  mounted() {
    this.$store.getters.fbUiApp.start('#firebaseui-auth-container', uiConfig);
  },

  destroyed() {
    this.$store.getters.fbUiApp.reset();
  },
};
</script>
